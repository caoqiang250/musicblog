	...
	{	
		...
		var Nostalgia=function(options,page,slide,audio,config,configDefault)
		{
			...
			this.createSupersizedSlider=function()
			{
				...
				//切换上一首
				$('#prevslide').bind('click',function(e)
				{
					if($this.index==0){//第一首向前到最后一首
						$this.index = $this.audioLength-1;
					}else{
						$this.index--;
					}
					$('#jPlayer').jPlayer('setMedia',$this.audio[$this.index]).jPlayer('play');
					e.preventDefault();
				});
				//切换下一首
				$('#nextslide').bind('click',function(e)
				{
					if($this.index==$this.audioLength-1){//最后一首向后到最前一首
						$this.index = 0;
					}else{
						$this.index++;
					}
					$('#jPlayer').jPlayer('setMedia',$this.audio[$this.index]).jPlayer('play');
					//console.log($this.audio[$this.index]);
					e.preventDefault();
				});
			};
			...
			this.switchSlide=function()
			{
				...
				//此处不切换幻灯
				// if($this.currentKey=='main') slideImageIndexPage=$this.options.slideImageIndexPageHome;
			...
			//显示日历插件
			this.showcalendarBox=function(show)
			{
				$this.calendarBox.css('display',(show ? 'block' : 'none'));
			};
			...
			this.createMenuSlider=function()
			{
				...
				$this.nostalgiaNavigationMenuList.carouFredSel(
				{
					...
					items: 
					{
						visible		:	(!(($.browser.msie) && (parseInt($.browser.version)<=8)) && ($this.getWindowHeight()<=510)) ? 2 : 3,
						minimum		:	2,
						width		:	'300px'
					},
					scroll: 
			...
			this.createAudioPlayer=function()
			{
				...
				$('#jPlayer').jPlayer( 
				{
					...
					{
						//自动循环
						//$(this).jPlayer('setMedia',$this.audio[$this.index]).jPlayer('play');
						//自动播放下一首
